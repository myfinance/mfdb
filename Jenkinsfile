 ...
 stages{
   ...
   stage('deploy to Cluster'){
     ...
     steps {
       ...
       sh 'helm upgrade -i --cleanup-on-fail mfdb ./helm/mfdb/ -n ${TEST_NAMESPACE} --set repository=${DOCKER_REPO}${DOCKERHUB_USER}/${ORGANIZATION_NAME}- --set stage=test --set mfdb.db_url=${K8N_IP}'
